...
long CMap::AniLoop()
{
	for(int i=0; i<AniCellNum; i++)
	{
		...
		{
			m_nAniCount[i] = 0;
			if( ++m_nAniFrame[i] >= (int)m_vAniCell[i].size() )
			{
				m_nAniFrame[i] = 0;			//当前桢
			}
		}
...
void CMap::ShowMap(bool active,LPDIRECTDRAWSURFACE surf)
{
	...
	SortNpc();
	for(int n=0; n<(int)m_vSortNpc.size(); n++)
	{
		//插入的方式显示显示主角，实现遮挡关系
...
{
	for(int i=0; i<(int)m_vNpc.size(); i++)
	{
