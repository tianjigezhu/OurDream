...
void FindEdge(LPDIRECTDRAWSURFACE lpDDSPict, WORD Color, WORD CK)
{
	...
	if( BeginDraw(lpDDSPict) )
	{
		...
		for(i=0; i<Height; i++)
			...
			{
				n=Pitch*i+j;			//按顺序扫描点
				if( Pict[n]!=CK && ((n+1<Height*Pitch+Width&&Pict[n+1]==CK) || (n>0&&Pict[n-1]==CK) 
						||(n<Pitch*(Height-1)+Width &&Pict[n+Pitch]==CK )||(n>=Pitch&& Pict[n-Pitch]==CK )) )
				{
...
void Edge_Blt(LPDIRECTDRAWSURFACE SS,int x,int y, LPDIRECTDRAWSURFACE DS,RECT rcRect,WORD CK, WORD Color)
{
	...
	for(int i=0; i<rcRect.bottom-rcRect.top; i++)
	{
		for( int j=0; j<rectWidth; j++)
		{
			...
			if( Dest[t2]!=CK && ((t2+1< (rcRect.bottom-rcRect.top)*DestWidth + rectWidth&&Dest[t2+1]==CK) || (t2>0&&Dest[t2-1]==CK) 
						||(t2+DestWidth< (rcRect.bottom-rcRect.top)*DestWidth + rectWidth&& Dest[t2+DestWidth]==CK) || (t2>=DestWidth)&&Dest[t2-DestWidth]==CK ) )
				Sour[t1]=Color;
			t1++;
			t2++;
		}
